              ...
              {requests.map(r => (
                <div key={r._id} className="flex justify-between p-4 bg-muted/50 rounded-lg">
                  ...
                  <div className="flex items-center gap-2">
                    {r.status === 'fulfilled' && <><CheckCircle className="w-5 h-5 text-secondary" /><Badge variant="secondary">Réalisée</Badge></>}
                    {r.status === 'pending' && <><AlertCircle className="w-5 h-5 text-accent" /><Badge className="bg-accent text-accent-foreground">En attente</Badge></>}
                    {r.status === 'processing' && <><Clock className="w-5 h-5 text-primary" /><Badge variant="outline">En cours</Badge></>}
                    <Button size="sm" variant="outline">Détails</Button>
                  </div>
                </div>
              ))}
            </div>
            <Button className="w-full mt-4">Nouvelle demande</Button>
          </CardContent>
        </Card>
      </div>
    </div>
